/**
 * Downloads Bible source data: a pre-built zip of the bundled open/public-domain
 * translations from Cloudflare R2, plus the cross-references dataset from openbible.info.
 *
...
// Pre-built zip of the bundled translations, hosted on Cloudflare R2.
const SOURCES_ZIP_URL =
  "https://pub-82480c48c8b84f8ca5162b8d42bd99d0.r2.dev/bible-sources.zip"
...
async function downloadSourcesZip(): Promise<void> {
  ...
  // Validate that we got a real zip and not an HTML error page (happens if the
  // URL is wrong or the R2 bucket's public access is disabled).
  const head = new Uint8Array(await Bun.file(zipDest).slice(0, 4).arrayBuffer())
  if (!isZip(head)) {
    await rm(zipDest, { force: true })
    throw new Error(
      "The download URL returned a non-zip response (likely an HTML page). " +
        "Verify SOURCES_ZIP_URL is correct and that the R2 bucket's public " +
        "access is enabled."
    )
  }
