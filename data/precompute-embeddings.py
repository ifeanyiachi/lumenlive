#!/usr/bin/env python3
"""
Pre-compute verse embeddings for LumenLive.
...
def encode_with_sentence_transformers(texts):
    ...
    t0 = time.time()
    # CANONICAL PREFIX CONTRACT: no prompt / prompt_name is passed, so verses
    # are embedded with NO prefix. Qwen3-Embedding uses a symmetric no-prefix
    # contract; the Rust runtime (OnnxEmbedder) also applies no prefix, so these
    # embeddings stay in the same subspace as runtime queries. Do NOT add a
    # "passage:"/"query:" prefix or prompt_name here — it would silently degrade
...
def main():
    print(f"\n=== LumenLive Verse Embedding Pre-computation ===")
