...
export const useQueueStore = create<QueueState>((set, get) => ({
  ...
  addItem: (item) =>
    set((state) => {
      // Verse dedup only applies to scripture items — lexicon cards from the
      // same verse are distinct study words and must not be collapsed.
      if (!item.verses && item.kind !== "lexicon") {
