...
export function useRemoteControl() {
  useEffect(() => {
    ...
    async function setup() {
      ...
      // remote:schedule_next — advance to next schedule item
      const u9 = await listen("remote:schedule_next", () => {
        if (cancelled) return
        useScheduleStore.getState().nextItem()
      })
      unlisteners.push(u9)

      // remote:schedule_prev — go to previous schedule item
      const u10 = await listen("remote:schedule_prev", () => {
        if (cancelled) return
        useScheduleStore.getState().prevItem()
      })
      unlisteners.push(u10)

      // remote:schedule_goto — jump to a specific schedule item by index
      const u11 = await listen<string>("remote:schedule_goto", (event) => {
        if (cancelled) return
        const payload = parsePayload(event.payload)
        const index = payload?.index as number | undefined
        if (index === undefined) return
        useScheduleStore.getState().goToItem(index)
      })
      unlisteners.push(u11)

      // remote:alert — show alert by template name (quick-send)
      const u12 = await listen<string>("remote:alert", (event) => {
        if (cancelled) return
        const payload = parsePayload(event.payload)
        const name = payload?.name as string | undefined
        if (!name) return

        const { templates, showAlert } = useAlertStore.getState()
      ...
      // remote:alert_message — show alert with custom message
      const u13 = await listen<string>("remote:alert_message", (event) => {
        if (cancelled) return
        const payload = parsePayload(event.payload)
        const templateName = payload?.template as string | undefined
        ...
        if (template) {
          showAlert(template.id, message)
        }
      })
      unlisteners.push(u13)

      // remote:dismiss_alert — dismiss all active alerts
      const u14 = await listen("remote:dismiss_alert", () => {
        if (cancelled) return
        useAlertStore.getState().dismissAll()
      })
...
async function presentQueueItem(index: number) {
  try {
    ...
    useBroadcastStore
      .getState()
      .setLiveVerse(toVerseRenderData(verseToPresent, translation), "queue")
  } catch (e) {
...
function syncStatusSnapshot() {
  ...
  const activeItem =
    activeSchedule && schedule.activeItemIndex !== null
      ? activeSchedule.items[schedule.activeItemIndex]
      : null
  ...
  invoke("update_remote_status", {
    ...
    activeScheduleItem: activeItem?.label ?? null,
    activeAlerts: alerts.activeAlerts.map((a) => a.message),
  }).catch(() => {})
}
