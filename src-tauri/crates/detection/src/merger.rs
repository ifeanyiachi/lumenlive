...
/// Synced from the frontend confidence slider on hydration.
const DEFAULT_AUTO_QUEUE_THRESHOLD: f64 = 0.65;
...
mod tests {
    ...
    fn test_merger_auto_queue() {
        ...
        assert_eq!(results.len(), 1);
        // 0.96 >= 0.65 auto_queue_threshold and no cooldown yet
    ...
    fn test_merger_auto_queue_below_threshold() {
        ...
        assert_eq!(results.len(), 1);
        // 0.50 < 0.65 auto_queue_threshold
        assert!(!results[0].auto_queued);
    }
    ...
    /// The anti-flood cooldown must coordinate ACROSS detection channels: a
    /// direct auto-queue starts the cooldown, so an immediately-following
    /// semantic detection on the SAME merger must not also auto-queue. This is
    /// the behaviour that the unified merger guarantees — before unification the
    /// direct and semantic paths held independent cooldown clocks and could
    /// auto-queue back-to-back, flooding the operator.
    #[test]
    fn test_merger_cooldown_coordinates_across_channels() {
        let mut merger = DetectionMerger::new();

        // Direct hit auto-queues and arms the cooldown.
        let direct = vec![make_detection(
            43,
            "John",
            3,
        ...
        let first = merger.merge(direct, vec![]);
        assert_eq!(first.len(), 1);
        assert!(first[0].auto_queued, "direct hit should auto-queue");

        // A distinct, above-threshold semantic hit arriving within the cooldown
        // window must NOT auto-queue, because the same merger already displayed.
        let semantic = vec![make_detection(
            45,
            "Romans",
            8,
        ...
        let second = merger.merge(vec![], semantic);
        assert_eq!(second.len(), 1);
        assert!(
            !second[0].auto_queued,
            "semantic hit within cooldown must be suppressed by the shared cooldown"
        );
    }
}
