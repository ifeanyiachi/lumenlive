...
mod tests {
    ...
    #[test]
    // Guards the hand-tuned constants: these assert real invariants (positive
    // threshold, weights summing to ~1). clippy only flags them because the
    // operands are `const` — the guard is the point.
