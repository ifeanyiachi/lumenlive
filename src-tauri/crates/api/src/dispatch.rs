...
///
/// This trait keeps the `lumenlive-api` crate free of any `tauri` dependency,
/// following the pattern established by `lumenlive-stt` and `lumenlive-detection`.
...
impl CommandDispatcher {
    pub fn dispatch(
        ...
        match cmd {
            ...
            RemoteCommand::ScheduleNext => sink.emit_event("remote:schedule_next", "{}"),
            RemoteCommand::SchedulePrev => sink.emit_event("remote:schedule_prev", "{}"),
            RemoteCommand::ScheduleGoTo(idx) => {
                let payload = serde_json::json!({ "index": idx }).to_string();
                sink.emit_event("remote:schedule_goto", &payload)
            }
            RemoteCommand::Alert(name) => {
                let payload = serde_json::json!({ "name": name }).to_string();
                sink.emit_event("remote:alert", &payload)
            }
            RemoteCommand::AlertMessage { template, message } => {
                let payload = serde_json::json!({ "template": template, "message": message }).to_string();
                sink.emit_event("remote:alert_message", &payload)
            }
            RemoteCommand::DismissAlert => sink.emit_event("remote:dismiss_alert", "{}"),
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn dispatch_schedule_next_emits_event() {
        let sink = MockSink::new();
        CommandDispatcher::dispatch(&RemoteCommand::ScheduleNext, &sink).unwrap();
        assert_eq!(sink.event_count(), 1);
        assert_eq!(sink.last_event().0, "remote:schedule_next");
    }

    #[test]
    fn dispatch_schedule_prev_emits_event() {
        let sink = MockSink::new();
        CommandDispatcher::dispatch(&RemoteCommand::SchedulePrev, &sink).unwrap();
        assert_eq!(sink.event_count(), 1);
        assert_eq!(sink.last_event().0, "remote:schedule_prev");
    }

    #[test]
    fn dispatch_schedule_goto_emits_event_with_index() {
        let sink = MockSink::new();
        CommandDispatcher::dispatch(&RemoteCommand::ScheduleGoTo(3), &sink).unwrap();
        assert_eq!(sink.event_count(), 1);
        let (event, payload) = sink.last_event();
        assert_eq!(event, "remote:schedule_goto");
        assert!(payload.contains('3'));
    }

    #[test]
    fn dispatch_alert_emits_event_with_name() {
        let sink = MockSink::new();
        CommandDispatcher::dispatch(&RemoteCommand::Alert("Nursery".into()), &sink).unwrap();
        assert_eq!(sink.event_count(), 1);
        let (event, payload) = sink.last_event();
        assert_eq!(event, "remote:alert");
        assert!(payload.contains("Nursery"));
    }

    #[test]
    fn dispatch_alert_message_emits_event() {
        let sink = MockSink::new();
        ...
        assert_eq!(sink.event_count(), 1);
        let (event, payload) = sink.last_event();
        assert_eq!(event, "remote:alert_message");
        assert!(payload.contains('T'));
        assert!(payload.contains('M'));
    }

    #[test]
    fn dispatch_dismiss_alert_emits_event() {
        let sink = MockSink::new();
        CommandDispatcher::dispatch(&RemoteCommand::DismissAlert, &sink).unwrap();
        assert_eq!(sink.event_count(), 1);
        assert_eq!(sink.last_event().0, "remote:dismiss_alert");
    }
