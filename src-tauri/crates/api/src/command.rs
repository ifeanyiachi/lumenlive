...
pub enum RemoteCommand {
    ...
    ScheduleGoTo(u32),
    Alert(String),
    AlertMessage { template: String, message: String },
    DismissAlert,
}
...
impl fmt::Display for RemoteCommand {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            RemoteCommand::ScheduleNext => write!(f, "schedule_next"),
            RemoteCommand::SchedulePrev => write!(f, "schedule_prev"),
            RemoteCommand::ScheduleGoTo(idx) => write!(f, "schedule_go_to({idx})"),
            RemoteCommand::Alert(name) => write!(f, "alert({name})"),
            RemoteCommand::AlertMessage { template, message } => {
                write!(f, "alert_message({template}, {message})")
            }
            RemoteCommand::DismissAlert => write!(f, "dismiss_alert"),
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn enum_has_exactly_14_variants() {
        let cmds: Vec<RemoteCommand> = vec![
            ...
            RemoteCommand::ScheduleGoTo(0),
            RemoteCommand::Alert(String::new()),
            RemoteCommand::AlertMessage { template: String::new(), message: String::new() },
            RemoteCommand::DismissAlert,
        ];

        for cmd in &cmds {
            // Identical arms are intentional: this is a compile-time
            // exhaustiveness guard. Adding a variant fails to compile here,
            // forcing this test (and the count below) to be updated.
            ...
            match cmd {
                ...
                RemoteCommand::ScheduleNext => {}
                RemoteCommand::SchedulePrev => {}
                RemoteCommand::ScheduleGoTo(_) => {}
                RemoteCommand::Alert(_) => {}
                RemoteCommand::AlertMessage { .. } => {}
                RemoteCommand::DismissAlert => {}
            }
        }
    ...
    // --- New variant serde tests ---

    #[test]
    fn serialize_schedule_next() {
        let json = serde_json::to_value(&RemoteCommand::ScheduleNext).unwrap();
        assert_eq!(json, serde_json::json!({"command": "schedule_next"}));
    }

    #[test]
    fn serialize_schedule_go_to() {
        let json = serde_json::to_value(RemoteCommand::ScheduleGoTo(3)).unwrap();
        assert_eq!(json, serde_json::json!({"command": "schedule_go_to", "value": 3}));
    }

    #[test]
    fn serialize_alert() {
        let json = serde_json::to_value(RemoteCommand::Alert("Nursery".into())).unwrap();
        assert_eq!(json, serde_json::json!({"command": "alert", "value": "Nursery"}));
    }

    #[test]
    fn serialize_alert_message() {
        let json = serde_json::to_value(&RemoteCommand::AlertMessage {
            template: "Nursery".into(),
            message: "Room 3".into(),
        }).unwrap();
        assert_eq!(json["command"], "alert_message");
    }

    #[test]
    fn serialize_dismiss_alert() {
        let json = serde_json::to_value(&RemoteCommand::DismissAlert).unwrap();
        assert_eq!(json, serde_json::json!({"command": "dismiss_alert"}));
    }

    #[test]
    fn deserialize_schedule_next() {
        let cmd: RemoteCommand = serde_json::from_str(r#"{"command":"schedule_next"}"#).unwrap();
        assert_eq!(cmd, RemoteCommand::ScheduleNext);
    }

    #[test]
    fn deserialize_schedule_go_to() {
        let cmd: RemoteCommand = serde_json::from_str(r#"{"command":"schedule_go_to","value":5}"#).unwrap();
        assert_eq!(cmd, RemoteCommand::ScheduleGoTo(5));
    }

    #[test]
    fn deserialize_alert() {
        let cmd: RemoteCommand = serde_json::from_str(r#"{"command":"alert","value":"Nursery"}"#).unwrap();
        assert_eq!(cmd, RemoteCommand::Alert("Nursery".into()));
    }

    #[test]
    fn deserialize_dismiss_alert() {
        let cmd: RemoteCommand = serde_json::from_str(r#"{"command":"dismiss_alert"}"#).unwrap();
        assert_eq!(cmd, RemoteCommand::DismissAlert);
    }

    #[test]
    fn roundtrip_new_variants() {
        let variants = vec![
            RemoteCommand::ScheduleNext,
            RemoteCommand::SchedulePrev,
            RemoteCommand::ScheduleGoTo(7),
            RemoteCommand::Alert("Test".into()),
            RemoteCommand::AlertMessage { template: "T".into(), message: "M".into() },
            RemoteCommand::DismissAlert,
        ];
