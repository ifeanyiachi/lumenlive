...
///
/// Each term is wrapped in double quotes so tokens containing apostrophes
/// (e.g. `I'll`) are valid FTS5 phrase tokens rather than barewords, which
/// would otherwise cause `fts5: syntax error near "'"`.
...
impl BibleDb {
    ...
    pub fn search_verses(
        ...
        // Attached (downloaded) translations have no FTS5 index — the index
        // lives only in the bundled DB — so they fall back to a plain LIKE scan.
        // Per-file FTS for downloads is a follow-up.
        if !route.prefix.is_empty() {
            return self.search_verses_like(&route, query, limit);
        }
        let conn = self
            .conn
            .lock()
    ...
    /// Substring search fallback for attached translations, which lack an FTS5
    /// index. `%`/`_` in the query are escaped so they're matched literally.
    fn search_verses_like(
        &self,
        route: &crate::db::Route,
        query: &str,
        limit: usize,
    ) -> Result<Vec<Verse>, BibleError> {
        let escaped = query
        ...
        let limit_i64 = limit as i64;
        let rows = stmt.query_map(
            rusqlite::params![route.local_id, pattern, limit_i64],
            |row: &rusqlite::Row| {
                Ok(Verse {
                    id: row.get(0)?,
...
mod tests {
    ...
    #[test]
    fn and_query_quotes_apostrophe_tokens() {
        // Regression: contractions like "I'll" must be quoted, not bare,
        // or FTS5 raises `syntax error near "'"`.
        assert_eq!(
            build_and_query("well done and I'll give you rest"),
            "\"well\" \"done\" \"I'll\" \"give\" \"rest\""
        );
    }
