...
use lumenlive_api::{
    ...
    generate_auth_token, new_shared_status, start_http_server, start_osc_listener,
};
...
pub async fn get_osc_status(
    ...
    Ok(OscStatus {
        running: runtime.handle.as_ref().is_some_and(lumenlive_api::OscHandle::is_active),
...
pub struct HttpRuntime {
    ...
    bound_port: Option<u16>,
    /// Access token for the currently-running server, shown to the operator so
    /// they can enter it on a remote device. `None` when the server is stopped.
...
/// Result of starting the HTTP API server, returned to the frontend.
#[derive(serde::Serialize)]
pub struct HttpStartInfo {
    pub port: u16,
    /// Access token the remote device must present. Displayed to the operator.
    pub token: String,
}

/// Start the HTTP API server on the given port.
///
/// A random access token is generated and required on the `/status` and
/// `/control` endpoints, so binding to `0.0.0.0` (for phone/tablet remotes on
/// the LAN) does not expose unauthenticated control. The token is returned to
/// the frontend to display to the operator.
#[tauri::command]
pub async fn start_http(
    app: AppHandle,
    state: State<'_, Mutex<HttpRuntime>>,
    port: Option<u16>,
) -> Result<HttpStartInfo, String> {
...

    {
        ...
        runtime.handle = Some(result.handle);
        runtime.bound_port = Some(bound_port);
        runtime.auth_token = Some(token.clone());
    }

    log::info!("HTTP API server started on port {bound_port} (token auth enabled)");
    Ok(HttpStartInfo { port: bound_port, token })
}
...
pub async fn get_http_status(
    ...
    Ok(HttpStatus {
        running: runtime.handle.as_ref().is_some_and(lumenlive_api::HttpHandle::is_active),
        port: runtime.bound_port,
        token: runtime.auth_token.clone(),
    })
}
...
#[allow(
    ...
    reason = "args mirror the IPC status payload; folding them into a struct \
              would change the frontend invoke contract"
)]
pub async fn update_remote_status(
    ...
    active_schedule_item: Option<String>,
    active_alerts: Option<Vec<String>>,
) -> Result<(), String> {
    let status = {
        let runtime = state.lock().map_err(|e| e.to_string())?;
    ...
    if let Some(v) = active_schedule_item {
        snapshot.active_schedule_item = Some(v);
    }
    if let Some(v) = active_alerts {
        snapshot.active_alerts = v;
    }
...
pub struct HttpStatus {
    pub running: bool,
    pub port: Option<u16>,
    /// Access token for the running server, so the UI can redisplay it while
    /// the server stays up. `None` when stopped.
