...
use super::detection::{
    check_reading_mode, check_translation_command, check_verse_command, run_direct_detection,
            ...
            match event {
                ...
                TranscriptEvent::Final {
                    ...
                    if !transcript.is_empty() {
                        ...
                        // Check for spoken verse commands — relative steps
                        // ("next verse", "skip the next two verses") and absolute
                        // jumps ("go to the third verse", "verse 5"). Finals ONLY:
                        // verse commands aren't idempotent, so running them on
                        // volatile partials would double-fire when the final
                        // repeats the phrase.
                        check_verse_command(&event_app, &transcript);
